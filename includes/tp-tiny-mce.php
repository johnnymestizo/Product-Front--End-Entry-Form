...
class TP_Tiny_MCE {
	...
	*/
	...
	if ( $teeny ) {
		$mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold, italic, underline, blockquote, separator, strikethrough, bullist, numlist,justifyleft, justifycenter, justifyright, undo, redo, imathImage, imathLink, unlink') );
	...
	tinyMCEPreInit = {
		...
		mceInit : {<?php echo $mce_options; ?>,
		    setup : function(ed) {
		        // Add image button
		        ed.addButton('imathLink', {
		            title : '<?php _e("Add a link","tradr-products");?>',
		            image : '<?php echo TRADR_PRODUCTS_PLUGIN_URL;?>/images/imath-link2.gif',
		            onclick : function() {
		                // Add you own code to execute something on click
						oldtext = ed.selection.getContent();
						if(!oldtext)
						{
							alert("<?php _e('Please select some content for your link','tradr-products');?>");
							return;
						}else{
							siUrl = prompt("<?php _e('Add the url of your link','tradr-products');?>", "http://");
							ed.focus();
			                if(siUrl) ed.selection.setContent('<a href="'+siUrl+'" target="_blank">'+oldtext+'</a>');
						}
		            }
		        }),
				ed.addButton('imathImage', {
		            title : '<?php _e("Add an image","tradr-products");?>',
		            image : '<?php echo TRADR_PRODUCTS_PLUGIN_URL;?>/images/imath-image.gif',
		            onclick : function() {
		                // Add you own code to execute something on click
						/*img_post = prompt("<?php _e('Url to your image','tradr-products');?>", "http://");
						if(img_post){
						img_width = prompt("<?php _e('Width in pixels of your image (only numbers)','tradr-products');?>");
						max_width_img = 300;
						...
						tb_show("Add Image","<?php echo TRADR_PRODUCTS_PLUGIN_URL.'/includes/tp-add-media.php?tab=add&TB_iframe=true&amp;height=500&amp;width=640';?>");
		            }
		        });
		    }
		},
